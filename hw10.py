...
c = Circle(5)
print(f"Radius: {c.radius}, Area: {c.area:.2f}, Perimeter: {c.perimeter:.2f}")
c.radius = 10
print(f"New Radius: {c.radius}, New Area: {c.area:.2f}")

#2. Создай класс Vector с атрибутами x и y. Реализуй магические методы __add__ (сложение двух векторов), 
# __str__ (вывод в формате "Vector(x, y)"), и __eq__ (сравнение). Проверь: Vector(1, 2) + Vector(3, 4) должен давать Vector(4, 6).
...
t.fahrenheit = 100
print(f"After 100F -> C: {t.celsius:.2f}, K: {t.kelvin:.2f}")

#4. Используй @dataclass для создания класса Point с полями x: float и y: float. 
# Добавь метод distance_to(other: Point) - расстояние до другой точки. Затем создай дочерний @dataclass класс Point3D, 
# добавив поле z: float, и переопредели distance_to.
...
p1 = Point(0, 0)
p2 = Point(3, 4)
print(f"Distance 2D: {p1.distance_to(p2)}")

p3d_1 = Point3D(0, 0, 0)
p3d_2 = Point3D(1, 1, 1)
print(f"Distance 3D: {p3d_1.distance_to(p3d_2):.2f}")
